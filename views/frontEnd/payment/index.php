...
<div class="container mx-auto p-8">
    ...
    <div class="bg-gray-700 p-6 rounded-lg shadow-md">
        ...
        <div class="bg-gray-600 shadow-md rounded px-4 pt-6 pb-8 my-4">
            ...
            <div class="flex justify-end items-center mt-4 text-white ">
        ...
        <form action="/process_payment" method="POST">
            <div class="mb-4">
                ...
                <input type="text" id="address" name="address" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-200" placeholder="Enter your address">
            </div>
            <div class="mb-4">
                <label for="paymentType" class="block text-sm font-medium text-gray-100">Payment Type</label>
                <select id="paymentType" name="paymentType" required class=" text-gray-500 mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-200">
            ...
            <div id="creditCardFields" class="hidden">
                <div class="mb-4">
                    ...
                    <input type="text" id="cardName" name="cardName" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-200" placeholder="John Doe">
                </div>
                <div class="mb-4">
                    <label for="cardNumber" class="block text-sm font-medium text-gray-100">Card Number</label>
                    <input type="text" id="cardNumber" name="cardNumber" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-200" placeholder="1234 5678 9012 3456">
                </div>
                <div class="flex mb-4">
                    <div class="w-1/2 pr-2">
                        <label for="expiry" class="block text-sm font-medium text-gray-100">Expiry Date</label>
                        <input type="text" id="expiry" name="expiry" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-200" placeholder="MM/YY">
                    </div>
                    <div class="w-1/2 pl-2">
                        <label for="cvv" class="block text-sm font-medium text-gray-100">CVV</label>
                        <input type="text" id="cvv" name="cvv" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-200" placeholder="123">
                    </div>
                </div>
            </div>
...
<script>
    // Lắng nghe sự kiện thay đổi trên dropdown payment type
    document.getElementById('paymentType').addEventListener('change', function() {
        const creditCardFields = document.getElementById('creditCardFields');
        if (this.value === 'credit_card') {
            creditCardFields.classList.remove('hidden'); // Hiện các trường thông tin thẻ tín dụng
        } else {
            creditCardFields.classList.add('hidden'); // Ẩn các trường thông tin thẻ tín dụng
        }
    });
