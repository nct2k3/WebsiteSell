...
<body class="text-gray-200 min-h-screen py-6">
    <div class="container mx-auto px-4 sm:px-6 max-w-6xl">
        <div class="bg-gray-800 backdrop-filter backdrop-blur-sm bg-opacity-90 rounded-xl overflow-hidden shadow-2xl p-6 mb-8 animate__animated animate__fadeIn">
            <h1 class="text-3xl font-bold text-center mb-2 bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400">Danh Sách Sản Phẩm</h1>
            <p class="text-gray-400 text-center mb-6">Quản lý và chỉnh sửa thông tin sản phẩm</p>
            
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="md:col-span-1">
                    <form method="POST" action="?controller=homeManager" class="space-y-4">
                            ...
                            <div class="input-field rounded-lg">
                                <select id="productLine" name="productLine" required onchange="this.form.submit()" class="w-full bg-transparent text-indigo-200 p-3">
                                    <?php if (!empty($NameLine)): ?>
                                        <option value="<?php echo $NameLine['ProductLineName']?>" disabled selected class="bg-gray-800 text-gray-200"><?php echo $NameLine['ProductLineName']?></option>
                                    <?php else: ?>
                    ...
                    <?php if(!empty($data)): ?>
                        ...
                        <div class="product-card <?php echo ($item->Status==1) ? 'hidden-product' : ''; ?> bg-gray-700 bg-opacity-70 rounded-xl overflow-hidden shadow-md">
                            <div class="flex items-center p-4">
                                <img src="<?php echo $item->img ?>" alt="Sản phẩm" class="h-24 w-24 object-cover rounded-lg mr-4 flex-shrink-0">
                                <div class="flex flex-grow min-w-0">
                                    <div class="flex-grow product-info pr-4 min-w-0">
                                        <div class="flex items-center justify-between mb-1">
                                            <h3 class="text-lg font-semibold text-indigo-200 truncate w-64 "><?php echo $item->productName ?></h3>
                                            <?php 
                                            $NewStatus = "Đang bán";
                                            ...
                                            ?>
                                            <span class="status-badge <?php echo $statusBadge; ?> whitespace-nowrap ml-3">
                                                <?php echo $NewStatus ?>
                                            </span>
                                        </div>
                                        <span class="text-gray-400 text-sm">Mã: <?php echo $item->productID?></span>
                                        ...
                                        <div class="flex flex-wrap gap-x-4 text-sm text-gray-300 mt-2">
                                            <div class="flex items-center">
                                                <svg class="w-4 h-4 text-indigo-400 mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <div class="flex-shrink-0 flex items-center">
                                        <button 
                                            ...
                                            class="bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2 rounded-lg transition-colors duration-300 btn-glow">
